...
class ittt(object):
        ...
        if event_d is not None and track_l is not None:
            ...
            while True:
                ...
                if splitdist > 15.0:	# token minimum first inter
                    ...
                    break
                count += 1
            LOG.debug(u'Configured %r splits: %r',
                      len(self.splitlist), self.splitlist)
    ...
    def loadconfig(self):
        ...
        self.chan_A = strops.confopt_chan(cr.get(u'event', u'chan_A'), defchana)
        self.chan_B = strops.confopt_chan(cr.get(u'event', u'chan_B'), defchanb)
        ...
        self.precision = strops.confopt_posint(cr.get(u'event', u'precision'),3)
        ...
        if self.winopen:
            ...
            if fbib:
                ...
                if r is not None:
                    ...
                    self.meet.txt_postxt(4,0,u' '.join([placestr, bibstr,
                                                         namestr, clubstr]))
                    self.meet.txt_postxt(5,26,strops.truncpad(tmstr,
                                                                  12, u'r'))
            ...
            if bbib:
                ...
                if r is not None:
                    ...
                    self.meet.txt_postxt(4,42,u' '.join([placestr, bibstr,
                                                         namestr, clubstr]))
                    self.meet.txt_postxt(5,68,strops.truncpad(tmstr,
                                                12, u'r'))
            ...
            for r in self.riders:
                ...
                if not self.teamnames:
                    namestr = strops.truncpad(strops.fitname(r[COL_FIRSTNAME].decode(u'utf-8'),
                              r[COL_LASTNAME].decode(u'utf-8'), 20-len(clubstr))+clubstr, 20)
                else:
                    namestr = strops.truncpad(r[COL_FIRSTNAME].decode(u'utf-8'),
                                              20)
                placestr = u'   ' # 3 ch
                if r[COL_PLACE]:
                    placestr = strops.truncpad(r[COL_PLACE].decode(u'utf-8') + u'.', 3)
    ...
    def do_properties(self):
        ...
        if response == 1:       # id 1 set in glade for "Apply"
            ...
            if newdistance != olddistance:
                LOG.debug(u'Event distance changed from %r to %r',
        ...
        for r in self.riders:
            ...
            if self.onestart:
                ...
                if r[COL_FINISH] is not None:
                    time = (r[COL_FINISH]-r[COL_START]).truncate(self.precision)
    ...
    def lap_trig(self, sp, t):
        ...
        if self.timerwin and type(self.meet.scbwin) is scbwin.scbtt:
            ...
            if sp is self.fs:
                ...
                self.meet.txt_postxt(5, 8, strops.truncpad(rlbl,17)
                                          + u' ' + self.fs.get_time())
            else:
                self.meet.scbwin.setr2(rlbl)
                glib.timeout_add_seconds(4, self.clear_rank,
                                            self.meet.scbwin.setr2)
                self.meet.txt_postxt(5, 50, strops.truncpad(rlbl,17)
                                          + u' ' + self.bs.get_time())
    ...
    def settimes(self, iter, st=None, ft=None, lt=None, splits=None,
            ...
            for sid in splits:
                ...
                if sid in self.splitmap:
                    ...
                    if splits[sid] is not None:
                        ...
                        self.splitmap[sid][u'data'].insert(splitval, None,
                                 bib, prec=self.precision)
                else:
    ...
    def insert_split(self, sid, st, bib):
        ...
        if sid in self.splitmap:
            ...
            LOG.debug(u'No ranking for rider %r at unknown split %r',
                        bib, sid)
        return ret
    ...
    def catchrider(self, sp):
        ...
        if not self.difftime:
            ...
            if op.getstatus() not in [u'idle', u'finish']:
                ...
                if ri is not None:
                    self.settimes(ri, st=self.curstart,
                                      splits=op.splits, comment=u'caught')
            ...
            if sp.getstatus() not in [u'idle', u'finish']:
                ...
                if ri is not None:
                    self.settimes(ri, st=self.curstart,
                                      splits=sp.splits, comment=u'catch')
        ...
        if bib != u'':
            ...
            if r is not None and r[COL_BIB].decode(u'utf-8') != u'':
                if self.teamnames:
                    ...
                    ret = u' '.join([strops.truncpad(name, name_w, u'l'),
                    ...
                    ret = u' '.join([strops.truncpad(r[COL_BIB], 3, u'r'),
    ...
    def log_elapsed(self, bib, start, finish,
                          sid=None, prev=None, manual=False):
